"""
Parallel beam comparison focused on gravity effects.

This example compares the behavior of linear, nonlinear, and mixed beam models
with and without gravity effects, running simulations in parallel for
better performance. Gravity forces account for beam deformation and use
the physics-accurate GravityForce implementation.
"""
...
def main():
    """Main function to run parallel gravity effects beam comparison."""

    print("=" * 60)
    print("PARALLEL GRAVITY EFFECTS BEAM COMPARISON")
    print("=" * 60)
    print(f"Running on {cpu_count()} CPU cores")
    print(f"Simulating {N_SEGMENTS} segments for {T_FINAL} seconds")
    print("Gravity forces account for beam deformation and rotation")
    print("-" * 60)
    ...
    try:
        ...
        # Create visualization with custom styling for gravity focus
        print("Creating visualization...")
        fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 14))

        # Define custom force styles for gravity comparison
        gravity_force_styles = {"No Forces": "-", "Gravity": ":"}
        ...
        ax1.set_title("Gravity Effects on Beam Response (Parallel Computation)")
        ax1.grid(True)

        # Create lines for animation
        lines = {}
        for name, (x, y) in shapes.items():
            _, _, color, style = get_beam_type_and_style(name, gravity_force_styles)
            (line,) = ax1.plot(
        ...
        ax2.set_title("Beam Tip Response - Gravity Effects Comparison")
        ax2.grid(True)
        ax2.legend()
        ...
        ax3.set_title("FFT Analysis - Gravity Loading Effects")
